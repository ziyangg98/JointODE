...
fit_ode <- JointODE(
  ...
  init = "marginal",
  control = list(verbose = TRUE, maxit = 50)
)
summary(fit_ode)
```
...
# JointODE: conditional cumulative hazard H(tl+dt) - H(tl)
pred_ode <- predict(fit_ode)
ode_aucs <- sapply(landmarks, function(tl) {
  ids <- unique(pred_ode$id)
  scores <- sapply(ids, function(sid) {
    p <- pred_ode[pred_ode$id == sid, ]
    h1 <- p$cumhaz[which.min(abs(p$time - tl))]
    h2 <- p$cumhaz[which.min(abs(p$time - (tl + dt)))]
    h2 - h1
  })
  landmark_auc(scores, ids, survival_data, tl, dt)
})

# Extended Cox: conditional cumulative hazard
