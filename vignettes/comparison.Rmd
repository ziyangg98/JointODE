...
long_data_surv <- long_data %>%
  left_join(surv_data %>% select(id, status), by = "id") %>%
  mutate(event_status = factor(status,
    levels = c(0, 1),
...
p_biomarker <- ggplot() +
  geom_line(
    data = long_data_surv, aes(x = time, y = observed, group = id, color = event_status),
    alpha = 0.15, linewidth = 0.15
  ) +
  geom_ribbon(
    data = avg_trajectories,
    aes(
      x = time, ymin = mean_obs - 1.96 * se_obs, ymax = mean_obs + 1.96 * se_obs,
      fill = event_status
    ), alpha = 0.2, color = NA
  ) +
  geom_smooth(
  ...
  scale_color_manual(values = c("Censored" = "#2E86AB", "Event" = "#E63946"), name = "Status") +
  scale_fill_manual(values = c("Censored" = "#2E86AB", "Event" = "#E63946"), name = "Status") +
...
p_velocity <- ggplot(
  ...
  geom_ribbon(aes(ymin = mean_vel - 1.96 * se_vel, ymax = mean_vel + 1.96 * se_vel),
    alpha = 0.15, color = NA
  ) +
...
set.seed(42)
example_ids <- sample(patient_params_valid$id, min(4, nrow(patient_params_valid)))
plot_data <- long_data %>%
