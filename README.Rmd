...
with initial conditions $m_i(0) = m_{0,i}$ and $\dot{m}_i(0) = v_{0,i}$, where $\omega_i$ is the natural frequency, $\xi_i$ is the damping ratio, and $f_i(t)$ is covariate-driven forcing. The dynamic parameters are estimated on log-coefficient scales: `biomarker` is $\log \omega_i^2$ and `velocity` is $\log(2\xi_i\omega_i)$. Individual heterogeneity is captured through random effects on initial states $(m_{0,i}, v_{0,i})$ and ODE parameters.
...
- **`(biomarker + velocity | id)`**: adds subject-specific random effects on these ODE parameters
