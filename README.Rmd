...
with initial conditions $m_i(0) = m_{0,i}$ and $\dot{m}_i(0) = v_{0,i}$, where $\omega_i$ is the natural frequency, $\xi_i$ is the damping ratio, and $f_i(t)$ is covariate-driven forcing. The dynamic parameters are estimated on latent scales: `omega` is $\log \omega_i$ and `xi` is a Gaussian damping root with $\xi_i = \xi_{\mathrm{root},i}^2$. Individual heterogeneity is captured through random effects on initial states $(m_{0,i}, v_{0,i})$ and latent ODE parameters.
...
- **`omega`**: includes the latent log-frequency parameter; physical $\omega_i=\exp(\text{omega}_i)$
- **`xi`**: includes the latent damping-root parameter; physical $\xi_i=\text{xi}_i^2$
- **`(omega + xi | id)`**: adds subject-specific random effects on these latent ODE parameters
